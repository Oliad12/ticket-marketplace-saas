          ...
          <div className="flex-1 min-w-0">
                ...
                {!isPastEvent && !event.is_cancelled && (
                  <>
                    <Link
                      ...
                      className="shrink-0 flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors"
                    >
            ...
            <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-gray-50 p-3 rounded-lg">
                ...
                <p className="text-2xl font-semibold text-gray-900">
                  {event.is_cancelled
                    ? formatPrice(event.metrics.refundedTickets * event.price, event.currency)
              ...
              {!event.is_cancelled && !isPastEvent && event.metrics.reservedTickets > 0 && (
                <div className="bg-yellow-50 p-3 rounded-lg border border-yellow-100 col-span-2 md:col-span-4">
                  <div className="flex items-center gap-2 text-yellow-700 mb-1">
                    <Clock className="w-4 h-4" />
                    <span className="text-sm font-medium">Reserved (pending payment)</span>
                  </div>
                  <p className="text-2xl font-semibold text-yellow-800">
                    {event.metrics.reservedTickets}
                    <span className="text-sm font-normal text-yellow-600 ml-1">
                      ticket{event.metrics.reservedTickets !== 1 ? "s" : ""} held — expires in ~15 min
                    </span>
                  </p>
                </div>
              )}
