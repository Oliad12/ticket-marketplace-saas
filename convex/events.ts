...
// Plain helper for checking availability — usable inside mutations/queries
...
export const create = mutation({
  args: {
    ...
    userId: v.string(),
    currency: v.optional(v.union(v.literal("etb"), v.literal("gbp"), v.literal("usd"))),
...
export const purchaseTicket = mutation({
  ...
  handler: async (ctx, { eventId, userId, waitingListId, paymentInfo, paymentProvider }) => {
  ...
  handler: async (ctx, { userId }) => {
    ...
    const eventsWithMetrics = await Promise.all(
      events.map(async (event) => {
        ...
        // Count active reserved offers (offered but not yet purchased)
        const now = Date.now();
        const waitingEntries = await ctx.db
          ...
          .collect();
        const reservedTickets = waitingEntries.filter(
          (e) => (e.offerExpiresAt ?? 0) > now
        ).length;
