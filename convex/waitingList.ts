...
/**
 * Internal helper (plain function) to process the queue.
 * Shared by processQueue mutation, expireOffer, cleanupExpiredOffers, and releaseTicket.
 */
...
export const cleanupExpiredOffers = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const [eventId, offers] of Object.entries(grouped)) {
      ...
      await processQueueHelper(ctx, eventId as Id<"events">);
    }
  },
});
...
export const getQueuePositionById = query({
...
export const storeChapaRef = mutation({
  args: {
    waitingListId: v.id("waitingList"),
    txRef: v.string(),
  },
  handler: async (ctx, { waitingListId, txRef }) => {
    await ctx.db.patch(waitingListId, { chapaTransactionRef: txRef });
  },
});
...
export const getByTxRef = query({
  ...
  handler: async (ctx, { txRef }) => {
    return await ctx.db
      .query("waitingList")
      .filter((q) => q.eq(q.field("chapaTransactionRef"), txRef))
      .first();
  },
});
